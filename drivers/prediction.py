...
class Prediction:
    def __init__(self):
        ...
        self.param_model_map = {"arima": "ARIMA",
    ...
    def plot_output(self, data_points: int = 0, plot_width: int = 15, plot_height: int = 6):
        return self.__plot_output(data_points, plot_width, plot_height)

    def init_class_vars(self, prediction_type: str, dataset: pd.DataFrame, label: str, date_column: str,
                        data_points: int, plot: bool):
        ...
        if isinstance(dataset, pd.Series):
            ...
            raise ValueError(
                "The dataset should either be a Dataframe or Series.")
    ...
    def __createmodel(self, model_type):
        ...
        if model_type == "ARIMA":
            ...
            prediction = self.input_df[self.label].shift(
                -self.data_points).to_numpy().reshape(-1, 1)
            X = self.input_df[self.label].to_numpy(
            ).reshape(-1, 1)[:-self.data_points]
            ...
            model.fit(X, y)
            X_predict = self.input_df[self.label].to_numpy(
            ).reshape(-1, 1)[-self.data_points:]
    ...
    def __plot_output(self, data_points: int, plot_width: int = 15, plot_height: int = 6):
        if data_points == 0:
        ...
        if self.plot:
            ...
            return self.plot_df.tail(line_nos).plot.line(figsize=(plot_width, plot_height))
        else:
            raise ValueError("Rerun the prediction with plot set to True")

    def __melt_output(self, predicted_cols, output_dfs):
        final_df = pd.concat(output_dfs, ignore_index=False, axis=1, )
        for col in predicted_cols:
            final_df[col] = final_df[col].fillna(
                value=self.input_df[self.label])
    ...
    def predict(self, prediction_type: str, dataset: pd.DataFrame, label: str, date_column: str, data_points: int = 20,
                plot: bool = True):
        # initializes the class variables
        self.init_class_vars(prediction_type=prediction_type, dataset=dataset, label=label, date_column=date_column,
                             data_points=data_points, plot=plot)
