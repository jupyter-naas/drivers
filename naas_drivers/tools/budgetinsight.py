...
class Connectors(BudgetInsight):
    ...
    def get_referential(self):
        ...
        df = df[["id", "name"]]
        df = df.rename(columns={"id": "id_bank",
                                "name": "bank_name"})
        return df
...
class Connections(BudgetInsight):
    ...
    def delete(self, connection_id):
        ...
        try:
            ...
            print(f"❌ Connection (id={connection_id}) does not exist. Please enter a valid id.")
            return self.get()
    ...
    def get_referential(self):
        ...
        df = df.rename(columns={"id": "id_connection",
...
class Accounts(BudgetInsight):
    ...
    def get_referential(self):
        ...
        # Create ref accounts
        ref_accounts = pd.merge(df_accounts,
                                ref_connections,
                                on=["id_connection", "id_user"])
        to_keep = ["id_user",
                   "id_connection",
                   "connection_state",
                   "connection_error",
        ...
        ref_accounts = ref_accounts.drop_duplicates(subset=['account_number']).reset_index(drop=True)
        return ref_accounts
    ...
    def get_statement(self):        
        ...
        # Create df positions
        df_statement = pd.merge(df,
                                ref,
                                on=["id_connection", "id_user", "id_account"])

        to_keep = ["id_connection",
                   "connection_state",
                   "bank_name",
                   "id_account",
                   "account_number",
        ...
        df_statement = df_statement.drop_duplicates(subset=['ACCOUNT_NUMBER']).reset_index(drop=True)
...
class Transactions(BudgetInsight):
    ...
    def get_statement(self):
        ...
        df_statement = df_statement[to_keep]
        df_statement.columns = df_statement.columns.str.upper()
        df_statement = df_statement.drop_duplicates(subset=['ACCOUNT_NUMBER', 'ID_TRANSACTION']).reset_index(drop=True)
        df_statement["DATE_EXTRACTION"] = datetime.now().strftime("%Y-%m-%d %H:%M")
        return df_statement
